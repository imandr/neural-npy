...
class GRU(Layer, ParamMixin):
    ...
    def init_layer(self, rng = None):
        ...
        self.Wxc = rng.normal(size = (hidden_size, in_size),
                scale= 1.0 / np.sqrt(in_size + hidden_size))
        self.Wxr = rng.normal(size = (hidden_size, in_size),
                scale= 1.0 / np.sqrt(in_size + hidden_size)) # input to reset
        self.Wxz = rng.normal(size = (hidden_size, in_size),
                scale= 1.0 / np.sqrt(in_size + hidden_size)) # input to interpolate

        # Recurrent weights
        self.Rhc = rng.normal(size = (hidden_size, hidden_size),
                scale= 1.0 / np.sqrt(hidden_size)) # hidden to candidate 
        self.Rhr = rng.normal(size = (hidden_size, hidden_size),
                scale= 1.0 / np.sqrt(hidden_size)) # hidden to reset 
        self.Rhz = rng.normal(size = (hidden_size, hidden_size),
                scale= 1.0 / np.sqrt(hidden_size)) # hidden to interpolate 

        # Weights from hidden layer to output layer
        self.Why = rng.normal(size = (out_size, hidden_size),
                scale= 1.0 / np.sqrt(hidden_size)) # hidden to output
      ...
      def forward(self, inputs, state = None):
        ...
        n, b, w = inputs.shape         # [sequence_length, batch_size, width]
        d = self.Nout
        ...
        for t in xrange(len(inputs)):
          ...
          hs[t] = cs[t]*zs[t] + hs[t-1]*(1.0 - zs[t])
