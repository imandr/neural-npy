...
class TrainingThread(Thread):
    ...
    def __init__(self, mgr, model, tdata, vdata, epochs, eta, 
    ...
    def validate(self, nsamples, tloss, terror):
        vx, vy_ = self.VData
        vy = self.Model.compute(vx)
        vloss = self.Model.display_loss(vy_)
        verror = self.Model.error(vy_)
        #print "verror, vloss=", verror, vloss
        self.Manager.addHistory(nsamples, tloss, terror, vloss, verror)
        self.Manager.validateCallback(nsamples, None, None, vx, vy_, vy, vloss, verror)
        

    def train_step(self, model, bx, by):
        eta = self.CurEta
        saved_params = model.params.copy()
        y, loss, terror = model.train(bx, by, eta, normalize_grads = self.NormalizeGrads)
        y1 = model(bx)
        loss1 = model.loss(by)
        if loss1 > loss:
            if self.CurEta > self.EtaMin:
                eta = max(self.EtaMin, eta*0.51)
                model.params = saved_params
                y, loss, terror = model.train(bx, by, eta, normalize_grads = self.NormalizeGrads)
                self.CurEta = eta
        else:
            self.CurEta = min(self.EtaMax, eta*1.12)
        print self.CurEta
        return y, loss, terror
        
    def train_step(self, model, bx, by):
        y, loss, terror = model.train(bx, by, self.Eta, normalize_grads = self.NormalizeGrads)
        return y, loss, terror
    ...
    def run(self):
        ...
        next_validate = 0 if self.ValidateSamples else None
        eta = self.Eta
        for epoch in range(self.Epochs):
            ...
            for bx, by in self.TData.batches(self.MbSize, randomize=self.Randomize, max_samples = self.EpochLimit):
                ...
                #print "calling train..."
                #y, tloss, terror = self.Model.train(bx, by, eta, normalize_grads = self.NormalizeGrads)
                y, tloss, terror = self.train_step(self.Model, bx, by)
                #print nepochsamples, tloss, terror
                dloss = self.Model.display_loss(by)
                nepochsamples += len(by)
                nsamples += len(by)
                #print "samples:", nsamples, " next report:", next_report
                if next_report != None and nsamples >= next_report:
                    #print "addHistory..."
                    self.Manager.addHistory(nsamples, dloss, terror, None, None)
                ...
                if next_validate != None and nsamples >= next_validate:
                    #print "validate..."
                    self.validate(nsamples, dloss, terror)
...
class Trainer:
    ...
    def __init__(self, train_generator, validate_data, test_data, model, port, callback_delegate=None):
    ...
    def startTraining(self, epochs, eta, 
        ...
        self.TThread = TrainingThread(self, self.Model, self.RData, self.VData,
